...
/// Only the GENERATED plist/unit can carry them — packaged units and
/// `brew services` cannot ([`propagates_xdg`] lets setup warn about that).
...
/// Whether the active backend can carry the current XDG overrides into the
/// daemon's environment. False for `brew services` and for a packaged
/// systemd unit — there the daemon will use the DEFAULT config/state paths.
pub fn propagates_xdg() -> bool {
    if xdg_env().is_empty() {
        return true; // nothing to propagate
    }
    #[cfg(target_os = "macos")]
    {
        // Brew's LaunchAgent is generated by brew, not us.
        matches!(brew_backend(), Ok(None))
    }
    #[cfg(target_os = "linux")]
    {
...
impl Backend for BrewServices {
    ...
    fn restart(&self) -> Result<()> {
        // Not `brew services restart`: for a job started with `run` it
        // preserves the transient, NOT-login-registered state. enroll_fresh
        // needs "registered and running the current config" — stop (ignore
        // "not running"), then a registering start.
        let _ = Command::new(&self.brew)
            .args(["services", "stop", "remux"])
            .output();
        run_out(Command::new(&self.brew).args(["services", "start", "remux"])).map(drop)
    }
}
...
impl SystemdUser {
    ...
    fn ensure_unit(&self) -> Result<()> {
        ...
        // Environment= values get the same treatment as ExecStart: systemd
        // gives quotes, backslashes, newlines and % special meaning, so a
        // path using them is refused rather than mis-quoted into the unit.
        ...
        for (k, v) in xdg_env() {
            ...
            {
                anyhow::bail!(
                    "{k}={v:?} contains characters unsafe in a systemd unit — \
                     install the unit manually (see packaging/remux.service)"
                );
            }
